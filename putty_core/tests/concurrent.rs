...
async fn bytes_from_two_independent_connections_do_not_get_mixed() {
    ...
    let (
        fake_connection_port_a,
        to_port_a_tx,
        _from_port_a_rx,
    ) = FakeConnection::new();

    let (
        fake_connection_port_b,
        to_port_b_tx,
        _from_port_b_rx,
    ) = FakeConnection::new();
    ...
    let packet_from_port_a =
        timeout(Duration::from_millis(100), receiver_port_a.recv())
            .await
            .expect("timed out waiting for PortA")
            .expect("broadcast channel for PortA closed unexpectedly");

    let packet_from_port_b =
        timeout(Duration::from_millis(100), receiver_port_b.recv())
            .await
